...
Shader "CG Shaders/Toon/CrossHatch"
{
	...
	Properties
	{
		...
		_outlineIntensityShadow ("Rim Shadow Offset", Range(-5.0, 2.5)) = 1.5
		_outlineIntensityShadow (" ", Float) = 1.5
		_rimColor("Outline Rim Color", Color) = (1,1,1,1)
		_hatchTexture("Hatching Texture", 2D) = "white" {}
		_hatchingOffset ("Hatching Offset", Range(-2.0, 1.0)) = 0
		_hatchingOffset (" ", Float) = 0
		_hatchingIntensity ("Hatching Intensity", Range(0.0, 3.0)) = 0
		...
		Pass
		{
			...
			fixed4 pShader(vert2Pixel IN): COLOR
			{
               ...
               // alternatively you can approximate deriving the z component without sqrt like so:  
				...
				//hatching
				fixed hatchLevel = saturate(dot(normalDir, lightDirection)+ _hatchingOffset) * 8;
				...
				fixed3 hatchingSample = tex2D(_hatchTexture, hatchUVs-_Time.x);
